...
We welcome contributions to the `azure-functions-db` project.
...
- Keep one representative example for the minimal DB trigger polling workflow.
- Keep one complex example for multi-table and SQLAlchemy-heavy behavior.
...
```bash
git commit -m "feat: add PostgreSQL polling support"
git commit -m "fix: handle duplicate watermark updates gracefully"
git commit -m "docs: improve quickstart documentation"
git commit -m "refactor: extract trigger lease coordination logic"
